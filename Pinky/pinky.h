...
namespace pinky {
	...
	extern std::atomic_bool isMHInitalized;

	/**
	 * Initalizes MinHook and Pinky (loads all resources and such) 
	 */
	extern bool initalize(bool loadImages = false);

	/**
	 * Loads the image resources from the dll, should only be called 
	 * by initalize.
	 */
	extern bool loadDllResources();

	/**
	 * Data of the replacement image that is loaded from the resources entry
	 */
	extern BYTE* replacementImage;
	extern size_t replacementImageSize;
	...
	extern DWORD hookProcess(ProcessInfo* procInfo, uint64_t breakPointOffset, LPTHREAD_START_ROUTINE hookFunction, void* param, size_t paramSize);

	/**
	 * Returns true if the filename is in the list of relevant file, which should be
	 * replaced by pinky.
	 */
	extern bool isReplacementFile(const char* filepath);

	// Taken from: https://github.com/perceptron8/pinky.ext/blob/master/background.js
	constexpr size_t FILE_LENGTH = 45;
	constexpr size_t FILE_COUT = 108;
	constexpr std::array<char[FILE_LENGTH], FILE_COUT> PINKY_FILES = {
		// live
